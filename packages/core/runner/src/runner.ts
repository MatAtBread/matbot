...
export async function* runSession(opts: RunSessionOpts): AsyncIterable<PipelineEvent> {
  ...
  for (;;) {
    ...
    // Commit the assistant tool-call message together with its tool results (and any tool markers),
    // atomically: disk never holds a tool_use without its matching tool_result, so an interruption
    // while the model produces the next step can't poison the session, yet completed steps survive.
    await store.set(session.id, session);
  }
