 ...
 * "Remember this" skill, now firing the `remember_fact` tool instead of loading a skill. The skill's
 * *body* became the tool (see `remember/tool.ts`); only these conditions remain, as data.
 *
 * `augment` conditions judge the user message (a fact stated/corrected); `followup` conditions judge
 * the assistant response (the assistant promising to remember, or owning a mistake). The tool reads
 * whichever message fired it — see the tool's header.
 */
export const REMEMBER_CONDITIONS: TriggerCondition[] = [
  { kind: 'augment', rule: 'MATCH when the user message contains any specific name, number, date, location, relationship, or domain detail that is not general knowledge — including short declarative facts ("The database table is called X") and conversational statements that embed specific knowledge ("Where I live 25% of electricity comes from a small hydro station"). Exclude: greetings, opinions without factual content, and questions.' },
  { kind: 'augment', rule: "MATCH when the user is correcting a specific factual error in the assistant's previous response — stating that a value, name, number, or behaviour is wrong and providing the correct one. Exclude: discussing corrections as a concept, asking about error handling, or giving feedback on formatting/style." },
  { kind: 'augment', rule: 'MATCH when the user is stating a FACT they want retained permanently across conversations. The key test: is the user stating a fact, or requesting an action? Facts match. Do NOT match task instructions ("remember to restart the server"), recalls ("remember when you said..."), or context-keeping ("keep track of which files we\'ve edited").' },
  { kind: 'followup', rule: 'MATCH when the assistant explicitly promises to remember a specific fact or correction ("I\'ve noted that X is Y", "I\'ll remember that the table name is Z"). Exclude: generic acknowledgements ("Good", "OK", "Noted") used as conversation transitions, and confirmations of completed tasks.' },
  { kind: 'followup', rule: 'MATCH when the assistant explicitly takes responsibility for a mistake, error, or oversight — phrases like "my fault", "my mistake", "I was wrong", "that\'s on me", "I shouldn\'t have assumed", "I missed that", or any clear admission that the assistant made an avoidable error.\n\nDO NOT MATCH: generic politeness ("sorry about that" as a filler), apologies for system latency, or apologies for the user\'s inconvenience rather than the assistant\'s own error.\n\nWhen this fires, the mistake itself (the correct fact or behaviour the assistant should have known) should be the thing remembered, not the apology.' },
];
