...
A value becomes searchable once its tool call has finished a ROUND. Two things are therefore invisible here, and both read as \`vetoed\` ("this term appears nowhere") for a value you just fetched: results from tool calls issued in the SAME batch as this one, which are not in the session until the round closes; and results from tool calls made INSIDE a \`tool_function\` composition, which never enter the session at all — only the composition's own return value does. To trace a freshly-fetched value, return it from the composition, or re-check it on a later turn.
